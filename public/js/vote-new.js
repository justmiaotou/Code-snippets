...
var multiOption = $('.multi-option'),
    ...
    radioMulti = $('#radio-multi'),
    radioSingle = $('#radio-single');

radioMulti.on('change', function() {
    if (this.checked) {
        multiOption.css('display', 'inline-block');
    }
});
radioSingle.on('change', function() {
    if (this.checked) {
        multiOption.hide();
    }
});
...
voteForm.on('submit', function(e) {
    ...
    if (!radioMulti[0].checked && !radioSingle[0].checked) {
        popAlert.show('请选择您的投票类型为单选或多选');
