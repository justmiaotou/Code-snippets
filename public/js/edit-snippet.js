...
var pops = require('common-pops'),
    popConfirm = new pops.ConfirmPop(),
    popAlert = new pops.AlertPop();

var btnDelSnippet = $('#del-snippet');
btnDelSnippet.length && btnDelSnippet.on('click', function() {
    var _this = this;
    popConfirm.show('确定删除该条目？', function() {
        ajax.getJSON(_this.getAttribute('data-href'), function(data) {
            popConfirm.hide();
            if (data.status) {
                popAlert.show('删除成功！页面即将关闭！', function() {
                    window.close();
                });
            } else {
